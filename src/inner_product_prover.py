...
class NIProver:
    ...
    def prove(self):
        ...
        u_new = x * self.u
        if self.fast:
            Prov2 = FastNIProver2(self.g, self.h, u_new, P_new, self.a, self.b,
                                  self.group, self.transcript.digest)
        else:
            Prov2 = NIProver2(self.g, self.h, u_new, P_new, self.a, self.b,
                              self.group, self.transcript.digest)
...
class NIProver2:
    ...
    def prove(self):
        ...
        L = vector_commitment(self.g[np:], self.h[:np], self.a[:np],
                              self.b[np:]) + cl * self.u
        R = vector_commitment(self.g[:np], self.h[np:], self.a[np:],
                              self.b[:np]) + cr * self.u
        ...
        return NIProver2(gp, hp, self.u, Pp, ap, bp, self.group,
                         self.transcript).prove()
    ...
    def prove(self):
        ...
        while True:
            if len(ap) == len(bp) == len(gp) == len(hp) == 1:
                return Proof2(ap[0], bp[0], xs, Ls, Rs, self.transcript.digest,
                              self.init_transcript_length)
            np = len(ap) // 2
            cl = inner_product(ap[:np], bp[np:])
            cr = inner_product(ap[np:], bp[:np])
            L = vector_commitment(gp[np:], hp[:np], ap[:np],
                                  bp[np:]) + cl * self.u
            R = vector_commitment(gp[:np], hp[np:], ap[np:],
                                  bp[:np]) + cr * self.u
            Ls.append(L)
            Rs.append(R)
            ...
            gp = [
                x.inv() * gi_fh + x * gi_sh
                for gi_fh, gi_sh in zip(gp[:np], gp[np:])
            ]
            hp = [
                x * hi_fh + x.inv() * hi_sh
                for hi_fh, hi_sh in zip(hp[:np], hp[np:])
            ]
            ap = [
                x * ai_fh + x.inv() * ai_sh
                for ai_fh, ai_sh in zip(ap[:np], ap[np:])
            ]
            bp = [
                x.inv() * bi_fh + x * bi_sh
                for bi_fh, bi_sh in zip(bp[:np], bp[np:])
            ]
